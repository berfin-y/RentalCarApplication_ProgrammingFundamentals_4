        ...
        switch (info[0]) {
            case "Individual":
                if (info[1].startsWith("M")) {
                    IIndividual membership = new Membership(new String[] {info[1], info[2], info[3], info[4], info[5]});
                    if (membership.checkId()){
                        individuals.add(membership);
                    }
                }
                else{
                    Individual member = new Member(new String[] {info[1], info[2], info[3], info[4], info[5]});
                    if (member.checkId()){
